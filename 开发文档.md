我需要开发一个基于 .NET 10 Minimal API + Blazor 的 RTSP 录制服务端。
在正式写代码之前，请先充当 **高级 .NET 架构师**，阅读我的需求描述，并输出一份详细的 **《技术设计方案与开发路线图》**。

**重要约束：**
1.  **NativeAOT 必须支持：** 所有的技术选型（Json库、RTSP库、日志库等）必须对 NativeAOT 友好（Trimming Compatible）。
2.  **本地化参考：** 我提到的参考代码 `RtspRecorder` 使用了 `[这里填写参考代码使用的核心库，例如 RtspClientSharp 或 FFmpeg.AutoGen]`。请在设计中说明如何将其封装为服务。

**你需要输出的内容包括：**

**1. 技术选型分析**
*   列出所有计划使用的关键 NuGet 包，并解释为什么它们适合 NativeAOT（例如：使用 System.Text.Json 而非 Newtonsoft）。
*   前端与后端通信方案（SignalR 还是 WebSocket？是否兼容 AOT？）。

**2. 架构设计**
*   **项目结构：** 推荐的 Solution 结构（例如：Client, Server, Shared）。
*   **核心类设计：**
    *   `RecordingManager`：如何管理并发任务？如何处理开始/停止？
    *   `ConfigurationService`：如何保证多线程读写 json 文件的安全？
*   **数据流向图（文字描述）：** 从“用户点击开始”到“文件写入磁盘”的全流程。

**3. 关键逻辑伪代码**
*   请写出 `BackgroundService` 轮询检查任务的核心逻辑伪代码。
*   请写出如何在 Minimal API 中通过 NativeAOT 启动 Blazor 静态文件的逻辑。

**4. 潜在风险评估**
*   针对 NativeAOT 可能会遇到的具体坑（例如 Blazor 的互操作性、RTSP 库的反射依赖等）给出预警和解决方案。

**5. 开发步骤规划**
*   将开发过程分解为 5-6 个可执行的步骤（Step-by-step），以便我后续分批让你生成代码。
...
实现一个RTSP回放录制服务端，使用Minimal API+blazor实现一个前端页面。用户双击可执行程序后，提示用户访问前端页面。

可以禁用国际化，以NativeAOT友好为目标。

技术栈：
- 后端：.NET 10 Minimal API
- 前端：Blazor WebAssembly (Hosted in ASP.NET Core) —— 注意：为了兼容NativeAOT，请勿使用Blazor Server模式。
- 实时通信：WebSocket (SignalR 或 原生Websocket，优先SignalR但需配置AOT兼容)。
...
### APP配置文件和频道配置
APP配置文件：程序运行路径下的appsettings.json文件。
```json
{
    "port": 8080,
    "recordPath": "./records",
...
4. 未来的时间：如果用户输入了一个未来的时间，而当前时间还未到该时间，那么就等待到该时间再开始录制。同时为了避免回放内容还没有生成，强制等待30秒后再开始录制。比如用户输入了2023-12-31 23:59:59，而当前时间是2023-12-31 23:59:00，那么就等待到2023-12-31 23:59:59再开始录制，同时强制等待30秒后再开始录制。